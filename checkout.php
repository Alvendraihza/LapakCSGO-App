<?php
	...
	if (!isset($_SESSION["pelanggan"])) {
		echo "<script type='text/javascript'>alert('Silahkan Login!')</script>";
				...
				<div class="row">
					<div class="table table-responsive">
						...
						<form action="" method="post">
							<input type="hidden" name="pelanggan" value="<?php echo $_SESSION["pelanggan"]["
							nama_pelanggan"];?>">
							<input type="hidden" name="produk" value="<?php echo $data["id_produk"];?>">
						...
						<?php 
							...
							if (isset($_POST["pelanggan"])) {
								...
								$rkode = rand(1000,10000);
								$idp = $_POST["produk"];
								$waktu =  date('Y-m-d H:i:s',$time);

								//menyimpan data ke tabel pembelian
								$sql = "INSERT INTO tbl_pembelian(id_pelanggan,id_produk,tanggal_pembelian,waktu,total_pembelian,rkode) VALUES ('$id_pelanggan','$idp','$tanggal_pembelian','$waktu','$total_pembelian','$rkode')";
								$query = mysqli_query($konek,$sql) or die (mysqli_error($konek));
								//mendapatkan id pembelian yang baru saja terjadi
								$id_pembelian_barusan = $konek->insert_id;

								foreach ($_SESSION["keranjang"] as $id_produk => $jumlah) {
									$sql2 = "INSERT INTO tbl_pembelian_produk(id_pelanggan, id_pembelian, id_produk, jumlah) VALUES('$id_pelanggan','$id_pembelian_barusan','$id_produk','$jumlah')";
									$query2 = mysqli_query($konek,$sql2) or die (mysqli_error($konek));
								}
								//kosongkan keranjang

								unset($_SESSION["keranjang"]);

								//tampilan atau arahkan kehalaman nota, nota dari pembelian yang baru saja terjadi

								echo "<script type='text/javascript'>alert('Selamat pembelian berhasil di lakukan')</script>";
								echo "<script type='text/javascript'>location='nota.php?nota_id=$id_pembelian_barusan'</script>";
